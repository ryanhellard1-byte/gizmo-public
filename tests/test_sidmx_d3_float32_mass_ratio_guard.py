...
def main() -> int:
    assert accepted(OBSERVED_PHASE172_R0_RATIO), (
        OBSERVED_PHASE172_R0_RATIO,
        RATIO_TOL,
    )

    # The guard must remain fail-closed for ratios outside float32 storage
    # uncertainty.  These are intentionally much closer to 3 than the 1:1
    # negative control used on the AWS target machine.
