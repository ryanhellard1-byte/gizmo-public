...
def read_gadget_format1(path: Path) -> Snapshot:
    with path.open("rb") as fh:
        ...
        if variable_count:
            # GIZMO format-1 snapshots may insert ParticleChildIDsNumber and
            # ParticleIDGenerationNumber records between ParticleIDs and Masses.
            # ICs generated by Phase172 do not. Format-1 has no block labels, so
            # scan a small bounded number of post-ID records and accept the first
            # record that is a physically valid fixed-mass record for every
            # variable-mass particle type. This keeps both layouts readable while
            # rejecting zero/subnormal integer-ID records interpreted as floats.
            variable = None
            for _ in range(8):
                mass_rec = read_record(fh, path)
                if mass_rec is None:
                    break
                if len(mass_rec) not in (4 * variable_count, 8 * variable_count):
                    continue
                candidate = decode_float_record(mass_rec, variable_count, "mass candidate", path)
                if np.any(~np.isfinite(candidate)) or np.any(candidate <= 0.0):
                    continue
                if len(mass_rec) == 4 * variable_count and np.any(candidate < np.finfo(np.float32).tiny):
                    continue
                ok = True
                vcur = 0
                for t in range(6):
                    count = int(npart[t])
                    if not count or mass_table[t] != 0.0:
                        continue
                    block = candidate[vcur:vcur + count]
                    vcur += count
                    scale = float(np.max(np.abs(block)))
                    if scale <= 0.0 or float(np.max(block) - np.min(block)) > max(1e-12, 1e-6 * scale):
                        ok = False
                        break
                if ok and vcur == variable_count:
                    variable = candidate
                    break
            if variable is None:
                raise ProfileError(f"{path}: no valid variable-mass record found after particle IDs")
        else:
