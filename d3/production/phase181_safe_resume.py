#!/usr/bin/env python3
"""Phase181 evidence-attested entry point for the Phase175 safe-resume engine."""
...
import phase181_machine_evidence as evidence  # noqa: E402
    ...
    try:
        ...
        def load_campaign():
            manifest_path, rows = resume.p173.materialize_manifest(
                Path(".phase181") / "phase172_manifest.csv"
            )
            for row in rows:
        ...
        BridgeError,
        evidence.EvidenceGateError,
        resume.ResumeError,
        resume.p173.LaunchError,
        OSError,
        ValueError,
    ) as exc:
        print(json.dumps({"phase": 181, "status": "FAIL", "error": str(exc)}, indent=2), file=sys.stderr)
        return 2
